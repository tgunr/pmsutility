...
#define PRINT_CHAR(CHAR) \
  do { \
	byteString = [[NSString alloc ] initWithBytes:CHAR length:1 encoding:NSUTF8StringEncoding]; \
	[appendString stringByAppendingString:byteString]; \
	[byteString autorelease]; \
	 ptr++; \
	 total_printed++; \
	 continue; \
     } while (0)
...
#define PRINT_TYPE(TYPE) \
  do { \
	...
	*sptr = '\0'; /* NULL terminate sptr.  */ \
	byteString = [[NSString alloc ] initWithFormat:@"%d" arguments:TYPE length:1 encoding:NSUTF8StringEncoding]; \
	[appendString stringByAppendingString:byteString]; \
	[byteString autorelease]; \
	} while (0)

int
_doprnt (const char *format, va_list ap, NSString *logString)
{
