...
void
PMLOGPushVerbose(Boolean value)
{
...
// Old C style logging
void
PM_LogInternal(const char * file, const char * function, int inLevel, char * fmt, ...)
{
...
void
_PMLOGSELF(id self, NSString * format, ...)
{
...
void
_PMLOG(const char * file, const char * function,  int inLevel, NSString * format, ...)
{
	if (inLevel > gDebugLevel) {
		// The level is not high enough to be displayed, we're skipping this item.
		return;
...
void
_PMERR(const char * file, const char * function, int err, NSString * format, ...)
{
	NSString   * pathString = [NSString stringWithCString: file encoding: NSUTF8StringEncoding];
	const char * fileString = [[pathString lastPathComponent] cStringUsingEncoding: NSUTF8StringEncoding];
